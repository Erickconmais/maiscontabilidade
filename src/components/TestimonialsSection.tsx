...
const TestimonialsSection = () => {
  ...
  return <section className="py-20 md:py-28 section-alt">
      <div className="container">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-6">
            ...
            <span className="text-highlight">dizem</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {testimonials.map((testimonial, index) => <div key={index} className="bg-card rounded-2xl p-6 border border-border relative">
              <Quote className="h-8 w-8 text-primary/20 absolute top-4 right-4" />
              
              <div className="flex gap-1 mb-4">
                {Array.from({
              length: testimonial.rating
            }).map((_, i) => <Star key={i} className="h-4 w-4 text-primary fill-primary" />)}
              </div>
